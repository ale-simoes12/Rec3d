...
if __name__ == '__main__':
    ...
    with open(output_file, 'w') as f:
        for filename in tqdm(filenames):
            ...
            raw_image = cv2.resize(raw_image, dimensoes2, interpolation=cv2.INTER_AREA) 
            ...
            if args.pred_only:
                ...
                #print("combined" , split_region )
                
                caption_space = np.ones((caption_height, combined_results.shape[1], 3), dtype=np.uint8) * 255
                captions = ['Raw image', 'Depth Anything']
                segment_width = w + margin_width
                #print("width" , segment_width )
                #print("width" , caption_space )
                ...
                cv2.imwrite(os.path.join(args.outdir, filename[:filename.rfind('.')] + '_img_depth.png'), depth)
